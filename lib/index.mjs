...
export function apply(ctx, config = {}) {
  ...
  const routes = [
    ...
    route('/api/data-ledger/client-width', async (req, res) => {
      if (!requireMethod(req, res, 'POST')) return
      const body = await readBody(req)
      const w = Number(body?.width)
      if (!Number.isFinite(w) || w < 100 || w > 2000) return json(res, 400, { ok: false, error: 'bad width' })
      const state = await readState(home)
      state.lastClientWidth = w
      await writeState(home, state)
      json(res, 200, { ok: true, lastClientWidth: w })
    }),
  ]
  ...
  const tools = [
    ...
    tool(
      'ledger_delete_paths',
      '数据台账管家：把指定路径批量移入回收站（一律可恢复）。铁律：必须先在对话里向用户逐条列出、得到用户明确同意后才能调用；任何"自动清理"都不允许。绝不触碰用户项目文件。',
      {
        type: 'object',
        properties: {
          paths: { type: 'array', items: { type: 'string' }, description: '要移入回收站的路径列表（来自 ledger_scan 的输出，需用户已明确同意）' },
        },
        required: ['paths'],
      },
      async (args) => {
    ...
    tool(
      'ledger_purge_paths',
      '数据台账管家：彻底删除回收站里【用户点名】的条目（不可恢复）。铁律：不存在"到期自动清空"；每次都必须先向用户列清单、用户点名同意后，只清点名的条目，未点名的一个都不动。',
      {
        type: 'object',
        properties: {
          paths: { type: 'array', items: { type: 'string' }, description: '用户明确点名要彻底删除的回收站条目路径列表' },
        },
        required: ['paths'],
      },
      async (args) => {
        const purged = [], failed = []
        for (const p of (args.paths ?? [])) {
          const r = await purgeTrash(home, p)
          if (r.ok) purged.push(p)
          else failed.push({ path: p, error: r.error })
        }
        return { ok: true, purgedCount: purged.length, failedCount: failed.length, purged, failed }
      },
    ),
    tool(
      'ledger_report_done',
