...
async function stewardScan(home) {
  ...
  const mainNames = new Set(sessions.filter(i => i.kind === 'main' || i.kind === 'fork').map(i => i.name))
  // 孤儿：主对话已不在磁盘上的子代理（第一档，可自动清）
  const orphans = sessions.filter(i => i.kind === 'subagent' && !mainNames.has(i.parentTitle))
  // 30 天未动的对话（第二档，需用户确认）
...
export function apply(ctx, config = {}) {
  ...
  const tools = [
    defineTool({
      name: 'ledger_scan',
      description: '数据台账管家：扫描 DSH 本地数据，返回总量、孤儿记录（主对话已归档的子代理）、30 天未动的对话、已卸载插件遗留数据、过期日志与回收站状态。用户问"产生了多少垃圾/占了多少空间"或每周例行巡检时使用。只读，不删除任何东西。',
      parameters: {},
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      async execute() { return await stewardScan(home) },
    }),
    defineTool({
      name: 'ledger_clean_orphans',
      description: '数据台账管家（第一档·自动清理，无需用户确认）：把孤儿子代理对话（主对话已被用户归档删除）、已卸载插件遗留数据、30 天前的过期日志移入回收站。全部可恢复（回收站保留 30 天）。绝不触碰用户的项目文件。执行后返回清理清单。',
      parameters: {},
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      async execute() {
    ...
    defineTool({
      name: 'ledger_delete_paths',
      description: '数据台账管家（第二档·需用户批准后调用）：把指定路径批量移入回收站。只能删除 ledger_scan 返回的、且用户已明确同意删除的数据（对话、数据文件、日志等）。绝不触碰用户项目文件。',
      parameters: {
        paths: { type: 'array', items: { type: 'string' }, required: true, description: '要删除的路径列表（来自 ledger_scan 的输出，需用户已批准）' },
      },
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      async execute(args) {
        const moved = [], failed = []
        for (const p of (args.paths ?? [])) {
          ...
          if (r.ok) moved.push({ path: p, trashPath: r.trashPath })
    ...
    defineTool({
      name: 'ledger_restore_path',
      description: '数据台账管家：把回收站里的条目恢复到原位置（用户反悔时使用）。',
      parameters: {
        path: { type: 'string', required: true, description: '回收站条目路径（来自 ledger_scan / 台账面板的回收站列表）' },
      },
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      async execute(args) { return await restoreFromTrash(home, args.path ?? '') },
    }),
    defineTool({
      name: 'ledger_purge_expired',
      description: '数据台账管家：彻底删除回收站里已超过保留期（默认 30 天）的条目。用户确认"回收站到期自动清空"后使用；未到期条目不动。',
      parameters: {},
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      async execute() {
        const inv = await collectInventory({ home, trashDays: current().trashDays })
    ...
    defineTool({
      name: 'ledger_report_done',
      description: '数据台账管家：记录"本周巡检汇报已完成"的时间戳（每周主动巡检节奏的锚点）。向用户汇报完巡检结果后调用一次。',
      parameters: {},
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      async execute() {
        const ts = Date.now()
