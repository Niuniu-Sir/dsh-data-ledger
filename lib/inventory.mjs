...
async function collectSessions(home) {
  ...
  for (const key of [...keys].sort()) {
    ...
    let summary = '', kind = 'unknown', parentTitle = null, care = null
    if (isSub) {
      kind = 'subagent'
      parentTitle = titleOf(header.parentSession)
      care = !onDisk.has(header.parentSession) ? 'archived' : null
    ...
    // 最近 24 小时还在活动的对话 → 慎重删除
    if (!care && st?.mtimeMs && Date.now() - st.mtimeMs < DAY) care = 'careful'
    items.push({
      name: titleOf(key),
      origin: originOf(dir),
      path: dir,
...
async function collectStorages(home) {
  ...
  for (const n of names.sort()) {
    ...
    items.push({
      ...
      mtime: st.mtimeMs,
      summary,
      care: ['track.json', 'session_projcache.json', 'workspace.json'].includes(n) ? 'careful' : null,
      deletable: true,
    })
  }
  return items
