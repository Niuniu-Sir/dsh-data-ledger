...
async function collectSessions(home) {
  ...
  for (const key of [...keys].sort()) {
    ...
    const title = entry?.rows?.title?.val || entry?.rows?.title || '(无标题)'
    const turns = entry?.rows?.sessionStats?.val?.turns
    ...
    if (dir) {
      ...
      items.push({
        ...
        summary: `${turns !== undefined ? turns + ' 轮' : '轮数未知'}${cwd ? ' · 工作区 ' + cwd : ''}${d.truncated ? '（大小近似）' : ''}`,
        deletable: true,
      })
    } else {
      items.push({
...
async function collectStorages(home) {
  ...
  for (const n of names.sort()) {
    ...
    items.push({
      ...
      mtime: st.mtimeMs,
      summary,
      deletable: n !== 'data-ledger-actions.log',
    })
  }
  return items
}
